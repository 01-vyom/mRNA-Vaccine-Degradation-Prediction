...
def predict(test, type=0, FOLD_N=5):
    ...
    for cv, val in enumerate([1, 2, 3, 4, 5]):
        print("Fold ", cv, "/", FOLD_N - 1, "...")
        model_short = build_model(seq_len=107, pred_len=107, type=type)
        model_long = build_model(seq_len=130, pred_len=130, type=type)

        model_short.load_weights(f"./models/model{Ver}{type}_cv{cv}.h5")
        model_long.load_weights(f"./models/model{Ver}{type}_cv{cv}.h5")
...
def __main__():
    ...
    for i in range(nmodel):
        public_df, public_preds, private_df, private_preds = predict(i)
