import numpy as np
import pandas as pd
from GRU_LSTM.data_processing import *
from GRU_LSTM.model_utility import *
...
def train_model(train, type=0, FOLD_N=5):
    ...
    for cv, (tr_idx, vl_idx) in enumerate(
        ...
        history = model.fit(
            ...
            callbacks=[
                ...
                tf.keras.callbacks.ModelCheckpoint(
                    f"./models/model{Ver}{type}_cv{cv}.h5"
                ),
            ],
        )
        print(
        ...
        model.load_weights(f"./models/model{Ver}{type}_cv{cv}.h5")
    ...
    plt.xlabel("epoch")
    plt.legend(plot_labels, fontsize="x-small")
    plt.savefig("GRU_LSTM_Model_" + str(type) + ".png", dpi=600)
    plt.show()
    return holdouts, holdout_preds, fold_val_losses


def run(train, nmodel=4, debug=False):
    val_df, val_preds, model_losses = [], [], []
    if debug:
        nmodel = 1
    for i in range(nmodel):
        holdouts, holdout_preds, fold_losses = train_model(i)
        val_df += holdouts
...
def __main__():
    ...
    val_df, val_preds, model_losses = run(train, 5, debug)
